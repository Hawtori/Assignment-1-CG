...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		scene->Lights[0].Position = glm::vec3(0.0f, 0.0f, 3.0f);
		...
		GameObject::Sptr monkey1 = scene->CreateGameObject("Spinny monkey");
		{
			// Set position in the scene
			monkey1->SetPostion(glm::vec3(1.5f, -5.0f, 1.0f));
			...
			RotatingBehaviour::Sptr rotation = monkey1->Add<RotatingBehaviour>();
			rotation->RotationSpeed = glm::vec3(1.f, 4.f, 7.f);
		}

		GameObject::Sptr monkey2 = scene->CreateGameObject("Back and forth monkey");
		{
			...
			monkeyMovement::Sptr movement = monkey2->Add<monkeyMovement>();
			movement->MoveSpeed = 3.f;
		}

		GameObject::Sptr monkey3 = scene->CreateGameObject("Square monkey");
		{
			monkey3->SetPostion(glm::vec3(-1.5f, -1.5f, 2.f));
			monkey3->SetRotation(glm::vec3(0.f));

			RenderComponent::Sptr renderer = monkey3->Add<RenderComponent>();
			renderer->SetMesh(monkeyMesh);
			renderer->SetMaterial(testMaterial);

			SquareMovement::Sptr movement = monkey3->Add<SquareMovement>();
			movement->MoveSpeed = 3.f;
		}
